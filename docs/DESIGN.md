...
- 縱深防禦：magic 亦在 HKDF `info` 內（§4.2），即使強行跳過檢查，派生出來的金鑰
  也不同，tag 不符。
...
header 的完整性完全由 `info` 承擔（§4.4）。`info` 與 AAD 兩者擇一即可，同時使用
不會提高保障，只會讓「哪個欄位保護哪一段」難以推理。
...
`AuthenticationTagMismatchException` 路徑，不需要任何額外程式碼。同一個機制也讓
magic 與 version 的竄改表現為認證失敗。
...
| `private.key` 已存在、互動環境、未帶 `-Force` | 印出現有金鑰的指紋與備份後的檔名，詢問是否繼續，預設為「不繼續」 | 覆蓋不再是不可逆的資料遺失（見下一列），但仍應讓使用者明確確認自己在動哪一把金鑰 |
...
4. 頂層 `catch` 以 `[Console]::Error.WriteLine` 印出單行錯誤訊息並 `exit 1`。
...
| 項目 | 決定 | 理由 |
|---|---|---|
| 原始碼檔案 | UTF-8 | 程式碼含大量中文錯誤訊息與 help。PowerShell 7 以 UTF-8 讀取無 BOM 檔沒有問題 |
...
**（1）容器 header 的完整性。**
保證：magic、version、contentType 三個欄位被竄改時，解密必然以 GCM 認證失敗告終，
不會產生任何輸出。機制是這三個欄位都進了 HKDF `info`（§4.2、§4.4）。
不保證：`ephPubKeyLen` 與 ephemeral 公鑰 DER 之外的長度欄位一致性——那些由解析階段
的長度檢查負責，錯誤表現為「容器格式錯誤」而非認證失敗。header 也沒有任何機密性，
它本來就是明文。
...
成功後才搬到正式位置；任何一步失敗都清掉暫存資料夾，`Destination` 不留半成品，
也不留暫存資料夾本身。
不保證：搬移階段是逐項 `Move-Item`，不是交易。搬移進行到一半時外力中斷（斷電、
強制終止）仍可能留下部分內容。`Destination` 內既有的同名檔案會被覆蓋。
...
乾淨。所有失敗一律 exit 1，並以單行訊息寫到 stderr。
不保證：外力中斷（強制終止行程）不在此保證範圍內。
...
對 repo 根目錄的兩支入口腳本執行的黑箱驗收套件，共 **90 案**。
...
全部。當前分佈為 Core 40 案 / Full-only 50 案。

**結構上的四條規矩：**

1. **受測腳本只在 `Invoke-Seal` / `Invoke-Open` 兩個函式裡各出現一次。** 案例一律
   透過這兩個語意動詞呼叫受測物，不自己拼參數陣列、不自己記得帶沙箱環境變數。
2. **「失敗案例必須同時成立」的紀律寫在 `Expect-SealRefused` / `Expect-OpenRefused`
   內部**，不做成呼叫端要記得加的開關。那組條件是：不得逾時（卡在互動提示等同永遠
   不會被發現的失敗）、必須真的失敗、不得留下產物、指定檔案不得被改動、訊息一律
   只比對 StdErr。最後一條是必要的：受測物的成功輸出含有與錯誤分類重疊的字樣
   （seal 每次都印「收件人公鑰指紋：RUNE-KEY …」），拿合併輸出比對會讓數種分類
   無條件命中。
...
`$env:HOME` 不會影響 `~` 的解析。每一次呼叫受測物之後都自動檢查使用者的真實家目錄，
偵測沙箱逃逸，不靠個別案例記得。
...
| 任何失敗 | exit 1；單行錯誤訊息寫到 stderr，不附呼叫堆疊 |
