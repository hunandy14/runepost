...
| 情境 | 行為 | 理由 |
|---|---|---|
| `private.key` 已存在 | **拒絕,不做任何事**(沿用原 811–813 行) | 覆蓋私鑰會讓已加密的舊檔案永久無法解密。這條資料遺失防護不變 |
| `private.key` 不存在、`public.pem` 存在 | **直接覆蓋 `public.pem`** | 孤兒 `public.pem`(私鑰已遺失)比沒有檔案**更危險** —— seal 會持續加密給一把沒人持有的金鑰,產出永久無法解讀的密文。覆蓋才是安全動作 |
...
讀取既有的 `private.key`,重新導出公鑰,**自由覆寫** `public.pem`,並印出 PEM 與指紋。
