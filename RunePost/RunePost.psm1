...
# 這兩行不是新行為，是把舊架構的既有行為搬過來。
# 舊的 dist/rune-*.ps1 在 param 區塊之後就 Set-StrictMode -Version Latest /
# $ErrorActionPreference = 'Stop'，整份腳本（含所有函式）都在其效力範圍內。
# 模組函式不繼承呼叫端的這兩項設定（實測：呼叫端設 Stop，模組函式仍讀到
# Continue；模組內設了才生效），入口腳本設定的版本管不到模組裡，所以必須在
# 模組範圍再宣告一次，否則「非終止錯誤不再終止」會讓大量既有錯誤路徑失效。
